...
public class LoginMaintenanceController extends WpBaseController {
	...
	public void testUpdateUser(HttpServletRequest request,HttpServletResponse response,String password){
		/*ResultJson rj = new ResultJson();
		//1.检验参数
		String phone = (String) SecurityUtils.getSubject().getSession().getAttribute(Const.PHONE_NUMBER);
		try {
			if (StringUtil.isBlank(password)||!password.matches("\\w{6,12}")) {
				throw new RuntimeException("请输入6~12位的密码");
			}
			if (StringUtil.isBlank(phone)) {
				throw new RuntimeException("找回失败");
			}
			SmUserTbl user = SmUserTbl.find.where().eq("username", phone).findUnique();
			if (user==null) {
				rj.setInfo("找回失败");
				rj.setResult(false);
				outJson(response, rj);
	...
	public void fetchPassword(HttpServletRequest request,HttpServletResponse response,String password,String checkCode,String phone){
		/*ResultJson rj = new ResultJson();
		//1.检验参数
		try {
		if(StringUtils.isBlank(password)||StringUtils.isBlank(checkCode)||StringUtils.isBlank(phone)){
				throw new ParameterException("参数错误");
		}
		Map<String,Object>  cMap = (Map<String, Object>) request.getSession().getAttribute(Const.SMS_VERIFY_CODE);
		if(!checkCode.equals(cMap.get("code"))||
				!phone.equals(cMap.get("phone"))){
	...
	public void auto_login(HttpServletRequest request,HttpServletResponse response,
										String devId,String userName,String password){
	  ...
	  public static  SmUserTbl auto_login_2(HttpServletRequest request,HttpServletResponse response,
			  								String devId,String token,String domain){
	 ...
	 * @author gongdzh   用户登录，当终端提交dev_id服服务端会生产token，
	 * 					token可作为下次一键登录使用
	 * 2016年12月26日 下午1:52:06
	 */
	@RequestMapping("/admin/login")	
	public void admin_login(HttpServletRequest request, 
							HttpServletResponse response,
							String userName,
							String password,
							String verifyCode,
							String weather,String modulus){		
		Session session = getSubjectSession();
		//校验
		ResultJson rj = new ResultJson();
...
//	    UsernamePasswordToken token = new UsernamePasswordToken(userName+"|"+instanceDomain, MD5.md5(password)); 
    	//由于需要兼容老版本的用户，这里试用另外一种md5加密方法 
		UsernamePasswordToken token = null;
        try {  
	...
	public ModelAndView pc_user_login(HttpServletRequest request, HttpServletResponse response,
