	...
	if($.Datatype){
		...
		$.extend($.Datatype,{
				...
				"needcheck":function(gets,obj,curform,regxp){
					var need=2;
					if(typeof(obj.attr("nc"))!="undefined" && obj.attr("nc")-0 > 0){
						need = obj.attr("nc")-0;
					}
					var	numselected=curform.find("input[name='"+obj.attr("name")+"']:checked").length;
					return  numselected >= need ? true : "选择"+need+"项！";
				},
				
				"maxcheck":function(gets,obj,curform,regxp){
					var atmax=2;
					if(typeof(obj.attr("mc"))!="undefined" && obj.attr("mc")-0 > 0){
						atmax = obj.attr("mc")-0;
					}
