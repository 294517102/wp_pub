...
//				创建数据
				function createAreaDate(w,h,x,y) {
				var data={};
				data.with=w;
				data.height=h;
				data.x=x;
...
//		                        $("#page_json").val("{'with':'"+Math.round(right-left)+"','height':'"+Math.round(bottom-top)+"',"+"'x':'"+Math.round(top)+"',"+"'y':'"+left+"'}");
//		                        console.log("系统坐标:"+mapPosition.attr('ref')+"数值：图片宽："+Math.round(right-left)+"图片高："+Math.round(bottom-top)+ "top："+Math.round(top)+"left:"+Math.round(left));
		                        ...
		                        $(setting.tag + '[name="areaItem"][ref='+ mapPosition.attr('ref') +']').attr("data",createAreaDate(Math.round(right-left),Math.round(bottom-top),Math.round(top),Math.round(left)));
		                        return false;   
