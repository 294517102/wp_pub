		...
		$.get('?mod=phpcms&file=more_pic_select&action=getdata&currentdir='+arr, function(data){
		    if(data !=null)
			{
				...
				$.each(arr_var, function(n){
					var val = arr_var[n];
					select += "<input type='hidden' name='"+textid+"[]' value='"+val+"'><div id='file_uploaded_1'><span style='width:30px'><input type='checkbox' name='"+textid+"_delete[]' value='"+val+"' title='É¾³ý'></span><span style='width:60px'><input type='text' name='"+textid+"_description[]' value='' size='20' title='Í¼Æ¬ËµÃ÷'></span><a href='###' onMouseOut='javascript:FilePreview(\""+val+"\", 0);' onMouseOver='javascript:FilePreview(\""+val+"\", 1);'>"+val+"</a></div>";
				});
				$('#'+textid).html(select);
			}
		});
