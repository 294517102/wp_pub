...
pinyin.prototype = 
{
        ...
        Trim: function(info){return info.replace(/(^\s*)|(\s*$)/g, "");},

        isEnKong1: function (argValue)
        {
                var flag=false;
        ...
        pinyin: function(char)
        {
                ...
                if (this.spellArray[char.charCodeAt(0)]) return spellArray[char.charCodeAt(0)]
                ascCode=toAsc(char)
                ascCode = eval("0x"+ascCode)
                if (!(ascCode>0xB0A0 && ascCode<0xD7FC)) return char
                for (var i=ascCode; (!this.spell[i] && i>0);) i--
                return this.spell[i]
        },
                ...
                if (str){
                        ...
                        for (var i=0; i<str.length; i++)
                        {
                                if (this.isEnKong1(str.charAt(i))) {
                                        ...
                                        pStr += " " + this.pinyin(str.charAt(i));
                                }
                        }
                        return this.Trim(pStr);
                }
        },

        toPinyinFirst: function(str)
        {
                if (str){
                        var pStr = ""
                        for (var i=0; i<str.length; i++)
                        {
                                if (this.isEnKong1(str.charAt(i))) {
                                        pStr += str.charAt(i).substr(0,1);
                                }
                                else{
                                        pStr += this.pinyin(str.charAt(i)).substr(0,1);
                                }
                        }
                        return this.Trim(pStr);
                }
