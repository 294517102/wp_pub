        ...
        $.ajax({
            ...
            success: function(result, op) {
                try {
                    ...
                    _input[0]=me.createOption("-请选择-","");
                    for (var i = 0; i < result.length; i++) {
                        _input[i+1] = me.createOption(result[i].k, result[i].v);
                        if (dv == result[i].v) {
...
function vbStyleSelectItem() {
    ...
    this.createHtmlObject = function (id) {
        ...
        var obj = this;
        _a.onclick = function () {
            var result = window.showModalDialog("/Admin/VisualTemplate/DataControlPanel/DataControlCssEditorEx.aspx?cmd=edit&ctr=" + escape(dc.fileName) + "&template=" + dc.template + "&style=" + _input.value + "&gp=" + dc.gp, "修改样式", 'scrollbars=no;resizable=no;help=no;status=no;center=1; dialogHeight=520px;dialogwidth=800px;');
...
function __OnChannelListCallback(v, t) {
    ...
    if (v) {
        if (__CallerID >= 0) {
            var item = BUILDER.getItem(__CallerID);
...
function CharFilter(s) {
    s = s.replace(/</ig, '&lt;').replace(/>/ig, '&gt;').replace(/"/ig, '&quot;').replace(/'/ig, '&acute;').replace(/&/ig, '&amp;').replace(/\s/ig, '&nbsp;');
    return s;
}

function CharConvert(s) {
    s = s.replace(/&lt;/ig, '<').replace(/&gt;/ig, '>').replace(/&quot;/ig, '"').replace(/&nbsp;/ig, " ").replace(/&acute;/ig, "'").replace(/&amp;/ig, '&');
    return s;
}
